...
class BaseEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            use_emb (bool): use addition speaker embedding information
        """
        ...
        # convert the [ speaker embedding, feat] --> new_feat 
        # only use if self.use_emb:
        self.linear_xs_embs = torch.nn.Linear(emb_dim+output_size,
                    output_size)
    ...
    def forward(
        ...
        # 2 layers: conv2d to do downsampling, 
        # import ipdb; ipdb.set_trace()
        # T --> 1/4 T;  80 mel --> 256 dim
        # B, T, mel 
        xs, pos_emb, masks = self.embed(xs, masks)
        mask_pad = masks  # (B, 1, T/subsample_rate)
        # addition subsampling for embeddings
        # print(self.use_emb, embs)
        if self.use_emb and embs is not None:
            # B, T, emb_dim
            assert self.emb_dim == embs.shape[-1]
            embs = (self.emb_dim**0.5)*embs/(torch.norm(embs, dim=2, keepdim=True)+1e-8)
            # do interpolation to match that with feat
            embs = embs.transpose(-2, -1)
            embs = F.interpolate(embs, size=[xs.shape[1]], mode='nearest')
            embs = embs.transpose(-2, -1)
            embs.detach()

            # concatenate xs and embs, 256+100-->256
            xs = self.linear_xs_embs(torch.cat((xs, embs), -1))
    ...
    def forward_chunk(
        ...
        for i, layer in enumerate(self.encoders):
            ...
            #print(f"new_cnn_cache: {new_cnn_cache}")
    ...
    def forward_chunk_by_chunk(
        ...
        for cur in range(0, num_frames - context + 1, stride):
            ...
            chunk_xs = xs[:, cur:end, :]
            #print(f"chunk_xs size: {chunk_xs.size()}")
...
class TransformerEncoder(BaseEncoder):
    ...
    def __init__(
        ...
        super().__init__(input_size, output_size, attention_heads,
                         ...
                         global_cmvn, use_dynamic_left_chunk, use_emb, emb_dim)
