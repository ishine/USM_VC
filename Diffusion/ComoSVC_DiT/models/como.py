...
from ComoSVC_DiT.models.estimator import Decoder
...
class Como(BaseModule):
    def __init__(self, total_steps,teacher = True ,dit_config=None):
        super().__init__()
        self.denoise_fn =Decoder(**dit_config)
    ...
    def EDMPrecond(self, x, sigma ,cond,denoise_fn,x_mask,condition):
        ...
        c_noise = sigma.log() / 4
        x = x.to(cond.dtype)
        c_in = c_in.to(cond.dtype)
        c_noise = c_noise.to(cond.dtype)
        if torch.isinf(c_in).any() or torch.isinf(c_noise).any() or torch.isinf(x).any():
            raise ValueError('inf in precond')
        F_x =  denoise_fn((c_in * x).transpose(1,2), x_mask,cond.transpose(1,2),c_noise.flatten(),condition).transpose(1,2)
        D_x = c_skip * x + c_out * (F_x .squeeze(1) ) 
        return D_x

    def EDMLoss(self, x_start, cond,x_mask,condition):
        ...
        D_yn = self.EDMPrecond(x_start + n, sigma ,cond,self.denoise_fn,x_mask,condition) # After Denoising
    ...
    def edm_sampler(self,latents, cond,num_steps=50, sigma_min=0.002, sigma_max=80, rho=7, S_churn=0, S_min=0, S_max=float('inf'), S_noise=1,x_mask=None,condition=None):
        ...
        for i, (t_cur, t_next) in tqdm(enumerate(zip(t_steps[:-1], t_steps[1:])),total = len(t_steps[:-1])):  
            ...
            denoised = self.EDMPrecond(x_hat, t_hat, cond, self.denoise_fn,x_mask,condition) # mel，sigma，cond
    ...
    def forward(self, x,x_mask, cond_ppg,condition, infer=False, source=None,sample_steps=30):
        if self.teacher: # teacher model  
            if not infer: # training
                loss = self.EDMLoss(x, cond_ppg,x_mask,condition)  # [10, 1843, 512],   [10, 1843, 1024]        
                return loss
            else: # infer
                shape = x.shape
                x = torch.randn(shape, device=cond_ppg.device).transpose(1,2)
                x=self.edm_sampler(x, cond_ppg, sample_steps,x_mask=x_mask,condition=condition)
            return x
