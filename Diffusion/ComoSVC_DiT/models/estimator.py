# from https://github.com/KdaiP/StableTTS/blob/main/models/estimator.py
import math
...
class DitWrapper(nn.Module):
    ...
    def forward(self, x, c, t, x_mask):
        x = (self.time_fusion(x, t) + x) * x_mask
...
class StyleAdaptiveLayerNorm(nn.Module):
    ...
    def forward(self, x, c): # 输入x为[B,T,D]
...
class Decoder(nn.Module):
    def __init__(self, hidden_channels, out_channels, filter_channels, dropout=0.05, n_layers=1, n_heads=4, kernel_size=3, gin_channels=0, mel_proj = False, mel_input_channels=80, mel_proj_channels = 256):
        super().__init__()
        ...
        if mel_proj:
            self.mel_porj = nn.Linear(mel_input_channels, mel_proj_channels)
        else:
            self.mel_porj = nn.Identity()
    ...
    def forward(self, x, mask, mu, t, c):
        ...
        x = self.mel_porj(x.transpose(1,2)).transpose(1,2)
